...
function renderData() {
    ...
    const currentMonthName = monthNamesFull[currentViewDate.getMonth()];
    ...
    const monthTx = transacciones.filter(tx => {
        if (!tx.Fecha) return false;
        const parts = tx.Fecha.split('/');
        return parts.length === 3 && parts[1] === mm && parts[2] === yyyy;
    });
    ...
    monthTx.forEach(tx => {
        if (tx.tipo === 'gasto') totalGastos += parseFloat(tx.Monto) || 0;
        if (tx.tipo === 'ingreso') totalIngresos += parseFloat(tx.Monto) || 0;
    });
    ...
    if (saldoEl) {
        saldoEl.innerText = `$${ahorroTotal.toFixed(2)}`;
    }
    ...
    renderTransactionsList(monthTx);
}
...
function renderTransactionsList(txList) {
    ...
    if (!txList || txList.length === 0) {
        container.innerHTML = '<p style="color: #77767D; text-align: center;">No hay transacciones registradas en este mes.</p>';
        return;
    }

    // Limitar a las ultimas 20 transacciones del mes
